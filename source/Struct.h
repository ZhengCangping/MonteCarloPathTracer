...
struct Material {
	...
	int map_id{-1};						// texture id:(0,texture_size-1) "map_id = -1" means no mapping txture
};
...
struct Triangle {
	...
	std::string texture;		// material name
	AABB aabb;
};
...
struct BVHNode {
	int left, right;
	int n, index;       // n:tri_num in leaf node    index:the first tri's index in all tris   
