...
int main()
{
    ...
    string model_n = a == 1 ? "veach-mis" : (a == 2 ? "bathroom" : "cornell-box");
    cout << line << endl;
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        shader1.use();
        // Attachment
        glBindFramebuffer(GL_FRAMEBUFFER, FBO1);    // put render result into update
        ...
        if (frameCounter % 100 == 0 && frameCounter < 1050) {
            string file = "./results/" + model_n + "_frame" + std::to_string(frameCounter)+".bmp";
...
// create a 2D GL_RGBA32F texture
unsigned int getTextureRGB32F(int width, int height) {
    unsigned int tex;
    glGenTextures(1, &tex);
