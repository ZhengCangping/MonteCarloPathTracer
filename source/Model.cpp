...
int Model::ReadMTL(const string& file) {
	...
	cout << "Texture library " << file << " has " << Texture.size() << " textures." << endl;
	string line(89, '-');
	cout << line << endl;
	return 0;
}
...
int Model::ReadXML(const string& file) {
	...
	while (getline(infile, data)) {
		...
		if (prefix == "mtlname") {
			...
			is >> mtlname;
			vec3 radiance;
			is >> radiance.x >> radiance.y >> radiance.z;
			Materials[mtlname].radiance = radiance;
		}
	}
	cout << "Successfully read file " << file << "." << endl;
	cout << "\t" << "img width:\t\t" << camera.width << endl;
	cout << "\t" << "img height:\t\t" << camera.height << endl;
	cout << "\t" << "lights num:\t\t" << Lights_num << endl;
	string line(89, '-');
	cout << line << endl;
	return 0;
}
...
void Model::Pre2Shader() {
	// prepare tri2shader,Lights2Shader
	Tris2Shader.clear();
	...
	cout << "Build BVH Finish!" << endl;

	// prepare Nodes2Shader
