...
uniform samplerBuffer Tris;         // GL_TEXTURE1
uniform samplerBuffer Tree;         // GL_TEXTURE2
uniform samplerBuffer Lights;       // GL_TEXTURE3

uniform sampler2D lastframe;        // GL_TEXTURE0
...
Light getLight(int i) {
    ...
    light.A = length(cross(light.v0 - light.v1, light.v2 - light.v1)) / 2;
    return light;
}
...
vec3 getTexture(int i, vec2 tex) {
    ...
    case 8:
        return texture2D(texture8, tex).rgb;
    case 9:
        return texture2D(texture9, tex).rgb;
    case 10:
        return texture2D(texture10, tex).rgb;
    default:
        return texture2D(texture0, tex).rgb;
    }
}
...
HitResult hitTriangle(Triangle triangle, Ray ray) {
    ...
    if (r1 || r2) {
        ...
        vec3 d3 = P-v2;
        vec3 n = cross(e1,e2);
        float an = n.x * n.x + n.y * n.y + n.z * n.z;
        float alpha = dot(cross(e1,d3),n)/an;
        float beta = dot(cross(e2,d1),n)/an;
        float gama = dot(cross(e3,d2),n)/an;
...
vec3 sampleLightSource(in vec3 x, Light lit,
    ...
    vec3 hitPoint = Xi1 * lit.v0 + Xi2 * lit.v1 + Xi3 * lit.v2;     //sample point in light tri
    vec3 dir = normalize(hitPoint - x);     //vector from shader point to light
    ...
    vec3 VN = alpha * lit.vn0 + beta * lit.vn1 + gamma * lit.vn2;   //normal of hitPoint
    float acos = dot(-dir, normalize(VN));
    if (acos <= 0.0) return vec3(0.0);      //unvisible
    sampleRec.w = dir;
    sampleRec.d = dist;
...
vec3 sampleLight(in HitResult hit,
                    in bool useMIS,
                    in int strataCount,
                    in int strataIndex) {
    ...
    for (int i = 0; i < DL_SAMPLES; i++) {
        ...
        for (int j = 0; j < nlights; ++j) {
            ...
            wo = rec.w;             //vector from shader point to light
            lightPdfW = rec.pdf;    //Monte Carlo PDF
            lightDist = rec.d;      //length of wo
            ...
            if ((dot(wo, ng) > 0.0) && (dotNWo > 0.0) && (lightPdfW > 0.00001f)) {
                ...
                if (dot(fr, fr) > 0.0) {
                    ...
                    shadowRay.origin = hit.hitPoint;
                    shadowRay.dir = wo;
                    if (isLightVisible(shadowRay, lightDist)) {//judge visibility
...
vec3 pathTracing(Ray ray, int maxBounce, float p) {
    ...
    vec3 Lo = vec3(0.0f); 
    vec3 history = vec3(1.0f); // the history of radiance
    ...
    for (int bounce = 0; bounce < maxBounce; bounce++) {
        ...
        if (hit.material.map_id != -1) {
            ...
            //float a = (hit.hitPoint.x-549)/7.0f;
            //float b = hit.hitPoint.y/550.0f;
            //float c = hit.hitPoint.z/560.0f;
            //return vec3(hit.texture.x,hit.texture.y,hit.texture.z);
        ...
        if (nlights > 10 && bounce != 0 && oo) {
            ...
            directLight = sampleLight(hit, test, frameCounter + 1, bounce);
        }
        ...
        if (!ishit) { break; }  // hit nothing or hit light, break
